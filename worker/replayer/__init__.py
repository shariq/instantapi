...
'''
    ...
    def list_all_droplets():
        ...
        response = requests.get(url, params={'page': page_number, 'per_page': 50}, headers=headers)
        response_json = response.json()
        droplets = [{'id': droplet['id'], 'name': droplet['name']} for droplet in response_json['droplets']]
...
class Pool:
    ...
    def rebalance(self):
        while True:
            if len(self.workers) < MIN_WORKERS:
                ...
                print('len(self.workers) < MIN_WORKERS; creating {} new workers'.format(new_workers))
                for i in range(new_workers):
                    self.create_worker()

            working_workers = [worker.status for worker in self.workers].count('working')
            utilization = float(working_workers) / len(self.workers)
            for worker in self.workers:
                if worker.status == 'available' and time.time() - worker.last_considered_destruction > 58 * 60:
                    ...
                    if utilization < SPIN_DOWN_THRESHOLD and num_workers > MIN_WORKERS:
                        print('destroying worker {}; utilization < SPIN_DOWN_THRESHOLD and num_workers > MIN_WORKERS'.format(worker))
            ...
            if utilization > SPIN_UP_THRESHOLD:
                ...
                while new_utilization > SPIN_UP_THRESHOLD:
                    new_workers += 1
                    new_utilization = float(working_workers) / (len(self.workers) + new_workers)
                print('utilization > SPIN_UP_THRESHOLD; creating {} new workers'.format(new_workers))
                for i in range(new_workers):
                    self.create_worker()
    ...
    def run_job(self, params):
        worker = self.get_free_worker() # returns None if no workers are available; should rarely ever happen
